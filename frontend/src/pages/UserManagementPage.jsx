...
// AdminLayout import removed — layout is provided by nested route in App.jsx
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3.5">
            <div className="flex items-start gap-4">
              <span className="text-[12px] font-semibold uppercase text-[#9CA3AF] shrink-0 pt-0.5" style={{ letterSpacing: '0.5px', minWidth: '96px' }}>
                Role
              </span>
              <RoleBadge role={drawerUser.role || 'USER'} userId={drawerUser.id} selfUser={false} disabled={isBusy} onChange={onRoleChange} />
            </div>
            <div className="flex items-center gap-4">
              <span className="text-[12px] font-semibold uppercase text-[#9CA3AF] shrink-0" style={{ letterSpacing: '0.5px', minWidth: '96px' }}>
                Status
              </span>
              <StatusBadge status={drawerUser.status || 'ACTIVE'} />
            </div>
            <div className="flex items-center gap-4">
              ...
              <span className="text-[14px] text-[#6B7280]">{formatDate(drawerUser.createdAt || drawerUser.joinedAt)}</span>
            </div>
          </div>
          <div className="border-t border-[#E5E7EB]" />
          <div>
            <div className="text-[12px] font-semibold uppercase text-[#9CA3AF] mb-3" style={{ letterSpacing: '0.8px' }}>
              ACTIONS
            </div>
            {isBusy && <p className="text-[13px] text-[#9CA3AF] mb-3">Saving…</p>}
            {drawerUser.status === 'PENDING' && (
              <button onClick={() => onApprove(drawerUser.id)} disabled={isBusy}
            ...
            {drawerUser.status === 'ACTIVE' && (
              <button onClick={() => onSuspend(drawerUser.id, drawerUser.name || drawerUser.email)} disabled={isBusy}
            ...
            {drawerUser.status === 'SUSPENDED' && (
              <button onClick={() => onReactivate(drawerUser.id)} disabled={isBusy}
            ...
            <button onClick={() => onDelete(drawerUser.id, drawerUser.name || drawerUser.email)} disabled={isBusy}
...
function UserManagementPage() {
  ...
  // Loading and error states — no AdminLayout wrapper, it comes from the route
  if (loading) return <div className="p-8 text-sm text-[#6B7280]">Loading users…</div>;
  if (error)   return <div className="p-8 text-sm text-[#DC2626]">{error}</div>;
  ...
  return (
    <div className="p-8 bg-[#F8FAFC] min-h-full" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {STATS_CARDS.map(s => (
          <div key={s.label} className="rounded-[10px] px-5 py-4 border-l-4" style={{ backgroundColor: s.bg, borderLeftColor: s.border }}>
            <div className="text-[28px] font-bold leading-none" style={{ color: s.numColor }}>{s.value}</div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#F8FAFC] border-b-2 border-[#E5E7EB]">
              {['User', 'Email', 'Role', 'Status', 'Joined', 'Actions'].map(col => (
                <th key={col} className="px-4 py-3 text-left text-[12px] font-semibold uppercase text-[#6B7280]" style={{ letterSpacing: '0.8px' }}>
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {filteredUsers.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-10 text-center text-[14px] text-[#9CA3AF]">No users found</td>
              </tr>
            ) : (
              filteredUsers.map(u => {
                ...
                return (
                  <tr key={u.id} className="border-b border-[#F3F4F6]"
                    style={{ height: '56px', backgroundColor: u.status === 'PENDING' ? '#FFFBEB' : 'white' }}>
                    <td className="px-4">
                      <div className="flex items-center gap-2.5">
                        {u.profilePicture ? (
                          <img src={u.profilePicture} alt="" className="w-9 h-9 rounded-full object-cover shrink-0" />
                        ) : (
                        ...
                        <div>
                          <div className="text-[14px] font-medium text-[#111827] leading-tight">{u.name || '—'}</div>
                          {isSelf && <div className="text-[11px] text-[#9CA3AF] leading-tight">You</div>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 text-[14px] text-[#6B7280]">{u.email}</td>
                    <td className="px-4">
                      <RoleBadge role={u.role || 'USER'} userId={u.id} selfUser={isSelf} disabled={updating === u.id} onChange={handleRoleChange} />
                    </td>
                    <td className="px-4"><StatusBadge status={u.status || 'ACTIVE'} /></td>
                    <td className="px-4 text-[13px] text-[#9CA3AF]">{formatJoined(u.createdAt || u.joinedAt)}</td>
                    <td className="px-4">
                      {isSelf ? (
                        ...
                        <div data-rowmenu>
                          ...
                          {openMenuId === u.id && createPortal(
                            ...
                            >
                              <button onClick={() => { setOpenMenuId(null); setSelectedUserId(u.id); }}
                              ...
                              {u.status === 'ACTIVE' && (
                                <button onClick={() => handleSuspend(u.id, u.name || u.email)}
